...
The entire project is written in python with commonly used packages like `pandas` and `numpy`. An external API [`JQData`](https://www.joinquant.com/help/api/help#name:JQData) has also been included for accessing Shen Wan (申万) industry category data (in lieu of GICS used by MSCI).
